...
while [ $n -lt ${nlines} ]; do
  ...
  if [ $4 = ${SCERN} ]; then
      ...
      cat submit_ZprimeMuMuAnalysis_FNAL.sh | sed "s?ZprimeMuMuAnalysis?ZprimeMuMuAnalysisMC?g" | sed "s?path?$PATH?g"  | sed "s?lib?$LD_LIBRARY_PATH?g" | sed "s?which?bkg?g" | sed "s?site?$4?g" | sed "s?mc?$3?g" |sed "s?year?$2?g" | sed "s?jobdir?jobs/jobsZprimeMuMu?g" | sed "s?histodir?histos/histosZprimeMuMu?g" | sed "s?output?output_${samplename}?g" | sed "s?bkg_input.txt?bkg_input_${n}.txt?g" | sed "s?sMC.log?s_${samplename}.log?g" > jobs/submit_ZprimeMuMuAnalysis_${samplename}.sh
      cat condor_templateMC.cfg  | sed "s?submit_ZprimeMuMuAnalysis_FNAL?submit_ZprimeMuMuAnalysis_${samplename}?g" | sed "s?sig_input_h150.txt?BkgCards$3/bkg_input_${n}.txt?g" | sed "s?mail?`whoami`?g" > jobs/condor_ZprimeMuMuAnalysis_${samplename}.cfg      
  elif  [ $4 = ${SDESY} ]; then
